...
## 🌍 Payment Integration for Mobile Apps

AfriNova supports **ALL 7 payment gateways** in mobile apps. Generate code based on your selected payment method:

### 1. Pesapal (Recommended for Africa - Mobile Money & Cards)

**Supports:** M-Pesa, Airtel Money, Tigo Pesa, Visa, Mastercard

```typescript
// Generated by AfriNova when you select Pesapal
import { WebView } from 'react-native-webview';

export async function initiatePesapalPayment(amount: number) {
...
**Setup:**
1. Get Pesapal API keys from pesapal.com
2. Add to your backend `.env`: `PESAPAL_CONSUMER_KEY`, `PESAPAL_CONSUMER_SECRET`
3. Test in sandbox mode first

---

### 2. Stripe (International Cards, Apple Pay, Google Pay)

```typescript
// Generated by AfriNova when you select Stripe
...
```typescript
// Generated by AfriNova when you select PayPal
import { PayPalButton } from '@paypal/react-native-paypal';

function PayPalScreen({ amount }: { amount: number }) {
  return (
    <PayPalButton
      amount={amount.toString()}
      currency="USD"
      onSuccess={(details) => console.log('Payment success')}
    />
  );
}
```
...
```typescript
// Generated by AfriNova when you select Flutterwave
...
### 5. Direct Mobile Money (M-Pesa, Airtel, MTN)

For specific mobile money providers:

```typescript
// Generated by AfriNova for direct mobile money
...
**AfriNova automatically generates the right payment code based on your tech stack selection!**
